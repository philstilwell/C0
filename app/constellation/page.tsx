...
export const metadata: Metadata = {
  ...
  description:
    "Five complementary papers on phenomenal presence, system boundaries, availability without report, scientific indeterminacy, and phenomenal character.",
  openGraph: {
    title: "The Cø / N* research constellation",
    description: "One core model, four complementary papers, and a shared discipline of falsifiability.",
  },
};
...
function MapNode({ id }: { id: keyof typeof paperById }) {
  ...
  return (
    <a className={`constellation-node node-${paper.tone}`} href={paper.href} aria-label={`Read ${paper.title} PDF`}>
      <span className="node-index">{paper.index} / {paper.type}</span>
      <strong>{id === "core" ? "Cø as N*" : id === "boundary" ? "Boundaries" : id === "availability" ? "Availability" : id === "indeterminacy" ? "Indeterminacy" : "Character"}</strong>
    ...
    <main className="constellation-page">
      ...
      <header className="constellation-hero" id="top">
        <div className="constellation-hero-copy">
          <p className="constellation-kicker"><span aria-hidden="true" /> FIVE PAPERS, ONE RESEARCH PROGRAM</p>
          <h1>One core claim.<br /><i>Four ways to keep it honest.</i></h1>
          <p>
            ...
            verdict, and then ask about the character of what is present.
          </p>
        ...
        <aside className="constellation-thesis" aria-label="The shared scientific commitment">
          ...
          <p>Each paper prevents a different shortcut: an unfixed system, a weak availability proxy, a forced binary verdict, or an unexplained leap from presence to character.</p>
        </aside>
      </header>
      ...
      <section className="constellation-map-section" id="map" aria-labelledby="map-heading">
        <div className="constellation-section-heading">
          ...
          <p>The central paper supplies the minimal criterion. Four companion papers reach into the conditions that make the criterion precise, testable, and appropriately limited.</p>
        </div>

        <div className="constellation-map" aria-label="A visual map of the five complementary papers">
          ...
          <p className="map-center-note">Each node is a paper. Follow any line to see the question it resolves.</p>
        </div>
      ...
      <section className="constellation-routes-section" id="how-they-fit" aria-labelledby="routes-heading">
        ...
        <div className="constellation-routes">
          ...
          <article>
            ...
            <p>First fix the system, then test availability without report, apply the conjunction, and preserve uncertainty when the evidence cannot settle the case.</p>
            <div><b>Boundaries</b><i>→</i><b>Availability</b><i>→</i><b>Cø / N*</b><i>→</i><b>Indeterminacy</b></div>
          </article>
          <article>
      ...
      <section className="constellation-papers-section" aria-labelledby="papers-heading">
        <div className="constellation-section-heading paper-list-heading">
          <div>
            <span>03 / FIVE DISTINCT JOBS</span>
            <h2 id="papers-heading">What each paper contributes.</h2>
          </div>
          <p>Read horizontally for the local question; read vertically to see the shared commitment to explicit scope, causal evidence, and falsifiable claims.</p>
        </div>
