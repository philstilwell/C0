---
title: "Reading Consciousness from the Schematic"
subtitle: "Structural Legibility in the $N^*$ Model"
...
$$
\mathcal D_E + \mathcal K_E + \mathcal I_E \vdash O_E.
$$

**Annotation.** $\mathcal D_E$ is the electrical diagram, $\mathcal K_E$ is knowledge of component laws and representational conventions, $\mathcal I_E$ is the declared input and operating condition, and $O_E$ is the inferred operating state. The turnstile marks a licensed derivation, not logical entailment from ink marks considered without semantics.
...
$$
\mathfrak S_{N^*}(S,c;\Theta)
=
\langle B_S,V,N_1,N_2,N_3\rangle,
$$

where $B_S$ records that $S$ passed an independent boundary criterion and $\Theta$ freezes the grain, horizon, regime, interventions, thresholds, and uncertainty policy. $B_S$ is not an additional phenomenal conjunct. It licenses the referent to which all the other terms apply.
...
A trained reader of $\mathfrak S_{N^*}$ does not jump directly from a Boolean vector to a phenomenal label. The reader reconstructs $\boldsymbol{\rho}_{C_0}$. Under the identity hypothesis, this is not a list of effects consciousness happens to cause. It is the proposed physical-role decomposition of minimal presence itself.
...
The phenomenal-character extension adds a second schematic layer. Once the presence gate is licensed, a content-specific causal-trajectory geometry $G_c$ may be mapped to an independently anchored quality space through a validated bridge $Q_\Phi$ (Stilwell, 2026e):

$$
V\land N^* \quad \text{licenses presence},
$$
...
$$
\operatorname{Legible}(M,o\mid K)
\Longleftrightarrow
D\land C\land F\land E.
$$
...
Stilwell, P. (2026a). *C0 as N\*: A minimal network-dynamics model of phenomenal consciousness* (Version 2.0) [Manuscript].
